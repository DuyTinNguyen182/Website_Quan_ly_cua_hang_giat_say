...
export default function KeLuuDoPage() {
  ...
  useEffect(() => {
    if (view === "list") fetchShelves();
  }, [view]);

  const fetchShelves = async () => {
    setLoading(true);
    try {
      const res = await axiosInstance.get("/shelves");
      setShelves(res.data);
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const openEdit = (shelf) => {
    setSelected(shelf);
    setForm({ name: shelf.name, note: shelf.note || "" });
    setError("");
    setView("edit");
  };
  ...
  const handleDelete = async (id) => {
    try {
      ...
      setShelves((prev) => prev.filter((s) => s._id !== id));
      setDeleteId(null);
    } catch (err) {
  ...
  const handleSubmit = async () => {
    if (!form.name.trim()) {
      setError("Vui lòng nhập tên kệ đồ");
      return;
    }
    setSubmitting(true);
    setError("");
    try {
      if (view === "add") {
        const res = await axiosInstance.post("/shelves", form);
        setShelves((prev) => [...prev, res.data.shelf]);
      } else {
        const res = await axiosInstance.put(`/shelves/${selected._id}`, form);
        setShelves((prev) =>
          prev.map((s) => (s._id === selected._id ? res.data.shelf : s))
        );
      }
      setView("list");
    } catch (err) {
            ...
            <div className="flex flex-col gap-6">
              ...
              <div className="flex flex-col gap-1">
                <label className="text-nav-bg text-sm font-medium">
                  Tên kệ đồ<span className="text-red-500">*</span>
                </label>
                <input
                  type="text"
                  value={form.name}
      ...
      <main className="flex-1 w-full max-w-300 mx-auto px-4 py-6 page-enter">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
          ...
          <div className="flex items-center gap-2 px-4 py-3 border-b border-gray-100">
            ...
            <input
              type="text"
              placeholder={`Tìm trong ${shelves.length} kệ đồ...`}
              value={search}
          ...
          <div className="grid grid-cols-[1fr_100px] px-4 py-2.5 bg-gray-50 border-b border-gray-100">
            <span className="text-xs font-bold text-gray-500 uppercase tracking-wide">Tên kệ đồ</span>
          ...
          {loading ? (
            ...
            <div className="py-14 text-center text-gray-400 text-sm">
              <span className="material-symbols-outlined text-4xl block mb-2 text-gray-300">shelves</span>
              {search ? "Không tìm thấy kệ đồ" : "Chưa có kệ đồ nào"}
            </div>
          ) : (
              ...
              <div key={shelf._id}>
                <div className="grid grid-cols-[1fr_100px] px-4 py-4 table-row-hover items-center">
                  <div>
                    ...
                    {shelf.note && (
                      <div className="text-[11px] text-gray-400 mt-0.5">{shelf.note}</div>
                    )}
                  </div>
      ...
      {deleteId && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center px-4">
          <div className="bg-white animate-scale-in rounded-2xl shadow-2xl w-full max-w-sm p-6">
            ...
            <h3 className="text-center font-bold text-gray-800 mb-2">Xóa kệ đồ?</h3>
